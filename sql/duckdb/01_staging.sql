/* ============================================================
   ...
   Purpose:
   1. Create cleaned staging views for the seven source tables.
   2. Exclude direct PII from downstream analytical use.
   3. Standardize names, types, blanks and text values.
   4. Derive sessions from events.csv.

   Important:
   - This does not use dim_sessions.csv
   - stg.sessions is derived from stg.events.
...
/* ============================================================
   ...
   Direct PII intentionally excluded:
   - first_name
   - last_name
   - email
   - street_address
...
SELECT
    ...
    UPPER(
        TRIM(CAST(gender AS VARCHAR))
    ) AS gender,

    NULLIF(
        TRIM(CAST(state AS VARCHAR)),
        ''
    ) AS state,

    NULLIF(
        TRIM(CAST(postal_code AS VARCHAR)),
        ''
    ) AS postal_code,

    NULLIF(
        TRIM(CAST(city AS VARCHAR)),
        ''
    ) AS city,
    ...
    COALESCE(
        NULLIF(
            TRIM(CAST(traffic_source AS VARCHAR)),
            ''
        ),
        'Unknown'
...
SELECT
    ...
    COALESCE(
        NULLIF(
            TRIM(CAST(department AS VARCHAR)),
            ''
        ),
        'Unknown'
    ) AS department,
    ...
    CAST(distribution_center_id AS INTEGER)
...
/* ============================================================
   ...
   Grain: one product line inside one order.
   ============================================================ */
...
/* ============================================================
   ...
   Grain: one website event.

   Direct PII intentionally excluded:
   - ip_address

   session_id and user_id are retained because they are needed
   for session and customer analysis, but they should not be
   displayed as public dashboard fields.
   ============================================================ */
...
SELECT
    ...
    NULLIF(
        TRIM(CAST(session_id AS VARCHAR)),
        ''
    ) AS session_id,
...
SELECT
    ...
    /* A valid session should normally contain only one user ID. */
    MAX(user_id) AS user_id,

    /* Total number of events in the session. */
    COUNT(*)::INTEGER AS event_count,

    /* Number of events that contain a user ID. */
    COUNT(user_id)::INTEGER AS events_with_user_id,

    /* Percentage of events connected to a known user ID. */
    COUNT(user_id)::DOUBLE
        / NULLIF(COUNT(*), 0)
    ...
    arg_min(traffic_source, sequence_number)
        AS traffic_source,

    /* First and final recorded event types. */
    arg_min(event_type, sequence_number)
        AS first_event_type,

    arg_max(event_type, sequence_number)
        AS final_event_type,

    /* Funnel flags: 1 means the event occurred at least once. */
    MAX(
        CASE
...
/* A missing session ID cannot be assigned to a real session. */
...
/* ============================================================
   ...
   Grain: one physical inventory item.
   ============================================================ */
...
SELECT
    ...
    COALESCE(
        NULLIF(
            TRIM(CAST(product_category AS VARCHAR)),
            ''
        ),
        'Unknown'
    ) AS category,
    ...
    COALESCE(
        NULLIF(
            TRIM(CAST(product_name AS VARCHAR)),
            ''
        ),
        'Unknown'
    ) AS product_name,
    ...
    COALESCE(
        NULLIF(
            TRIM(CAST(product_brand AS VARCHAR)),
            ''
        ),
        'Unknown'
    ) AS brand,

    TRY_CAST(product_retail_price AS DOUBLE)
        AS retail_price,

    COALESCE(
        NULLIF(
            TRIM(CAST(product_department AS VARCHAR)),
            ''
        ),
        'Unknown'
    ) AS department,
    ...
    CAST(product_distribution_center_id AS INTEGER)
        AS distribution_center_id
