/* ============================================================
   ...
   Purpose:
   Create reusable analytical fact tables at controlled grains.
...
SELECT
    ...
    CASE
        WHEN o.shipped_at >= o.created_at
        THEN
            datediff(
                'minute',
                o.created_at,
    ...
    CASE
        WHEN o.delivered_at >= o.shipped_at
        THEN
            datediff(
                'minute',
                o.shipped_at,
    ...
    CASE
        WHEN o.returned_at >= o.delivered_at
        THEN
            datediff(
                'minute',
                o.delivered_at,
    ...
    CASE
        WHEN o.shipped_at IS NOT NULL
             AND o.shipped_at < o.created_at
        THEN 0

        WHEN o.delivered_at IS NOT NULL
             AND (
                 o.shipped_at IS NULL
                 OR o.delivered_at < o.shipped_at
             )
        THEN 0
        ...
        WHEN o.returned_at IS NOT NULL
             AND (
                 ...
                 OR o.returned_at < o.delivered_at
             )
        THEN 0
    ...
    CASE
        ...
        THEN 1
        ELSE 0
    END AS returned_order_flag,

    CASE
        WHEN o.order_status = 'Complete'
        THEN 1
        ELSE 0
...
SELECT
    ...
    CASE
        WHEN i.item_status = 'Cancelled'
        THEN i.sale_price
        ELSE 0
    END AS cancelled_value,

    CASE
        WHEN i.item_status = 'Returned'
        THEN i.sale_price
        ELSE 0
    END AS returned_value,

    CASE
        WHEN i.item_status NOT IN (
            'Cancelled',
            'Returned'
        )
        THEN i.sale_price
    ...
    CASE
        ...
        THEN i.sale_price - p.unit_cost
        ELSE 0
    END AS net_profit_value,

    CASE
        WHEN i.item_status = 'Cancelled'
        THEN 1
        ELSE 0
    END AS cancelled_item_flag,
    ...
    CASE
        WHEN i.item_status IN (
            'Complete',
            'Returned'
        )
        THEN 1
        ELSE 0
    END AS return_observation_eligible_flag,
    ...
    CASE
        WHEN i.delivered_at >= i.shipped_at
        THEN
            datediff(
                'minute',
                i.shipped_at,
    ...
    CASE
        WHEN i.delivered_at >= i.created_at
        THEN
            datediff(
                'minute',
                i.created_at,
    ...
    CASE
        WHEN i.returned_at >= i.delivered_at
        THEN
            datediff(
                'minute',
                i.delivered_at,
    ...
    CASE
        WHEN i.shipped_at IS NOT NULL
             AND i.shipped_at < i.created_at
        THEN 0

        WHEN i.delivered_at IS NOT NULL
             AND (
                 i.shipped_at IS NULL
                 OR i.delivered_at < i.shipped_at
             )
        THEN 0

        WHEN i.returned_at IS NOT NULL
             AND (
                 i.delivered_at IS NULL
                 OR i.returned_at < i.delivered_at
             )
        THEN 0
...
SELECT
    ...
    CASE
        WHEN s.events_with_user_id > 0
        THEN 1
        ELSE 0
    END AS identified_session_flag,

    CASE
        WHEN s.session_end_at
             >= s.session_start_at
        THEN
            datediff(
                'second',
    ...
    CASE
        WHEN s.product_view_flag = 1
             AND s.cart_flag = 0
             AND s.purchase_flag = 0
        THEN 1
        ELSE 0
    END AS early_product_exit_flag,

    CASE
        WHEN s.purchase_flag = 1
            THEN 'Purchase'
...
WITH as_of AS (
    SELECT
        MAX(created_at)::DATE AS as_of_date
...
FROM stg.inventory_events AS i
